...
def _load_assignment_meta(db: Session, assignment_id: int) -> Dict[str, Any]:
    ...
    if not a.spec_url:
        raise HTTPException(
            status_code=400, detail="Assignment meta path (spec_url) is missing."
        )
    meta_path = Path(a.spec_url)
    if not meta_path.exists():
        raise HTTPException(
            status_code=400, detail=f"Assignment meta not found: {meta_path}"
        )
    try:
        return json.loads(meta_path.read_text(encoding="utf-8"))
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to read assignment meta JSON: {e}"
        )
...
def ai_init_from_assignment(
    ...
    if not candidate:
        raise HTTPException(
            status_code=400, detail="No valid spec/rubric path in assignment meta."
        )
...
def ai_run_predict(
    ...
    | None = Query(
        None,
        description="Optional: POST AI results back to /v1/marking_result/{course_id}/append",
    ),
    ...
    def _task():
        print(
            f"[AI][RUN] run_predict_pipeline(course_id={course_id}, backend_url='{backend_url}')"
        )
...
def ai_get_result():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to read AI result JSON: {e}"
        )
...
def ai_sync_predictions(
    ...
    if path:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to resolve default prediction path: {exc}",
            )
