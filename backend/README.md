...
1. **Install dependencies**
   ...
   ```
   Poetry enforces the project’s Python ≥3.12 requirement. If `poetry env use` fails, install Python 3.12 first (e.g., `brew install python@3.12` or `pyenv install 3.12.x`) and point Poetry to the binary on your system.
...
Use the Poetry environment to run the pytest suite:

When iterating on a specific helper or router you can target individual modules, e.g.:
...
Keep FastAPI `TestClient` suites beside the routers/services they exercise and expand coverage as you add new behaviours.
